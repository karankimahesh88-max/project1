...
def main():
    ...
    # ---------------- Top nav bar (logo + user + logout) ----------------
    nav_l, nav_r1, nav_r2 = st.columns([6, 3, 1.2])
    with nav_l:
        st.markdown('<div class="kb-logo">💰&nbsp;Finance&nbsp;Tracker</div>', unsafe_allow_html=True)
    with nav_r1:
        st.markdown(
            f'<div class="kb-user">Signed in as {user["email"]}</div>',
            unsafe_allow_html=True,
        )
    with nav_r2:
    ...
    tab_dash, tab_tx, tab_inv, tab_goals, tab_settings = st.tabs(
        ["Dashboard", "Transactions", "Investments", "Goals", "Settings"]
    )
